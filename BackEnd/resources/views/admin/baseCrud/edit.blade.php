                ...
                <div class="card-header">
                    <div class="row">
                        <div class="d-flex flex-wrap justify-content-between align-items-center">
                            <h5 class="card-title">Cập nhật {{ $title_web }}</h5>
                                ...
                                @foreach($colums as $key=>$item)
                                    ...
                                    @elseif(DESC_FIELD === $key)
                                        <div class="row">
                                            <div class="col-md-8 mb-3">
                                                <label class="mb-2" for="validationCustom01">{{ $item }}</label>
                                                <input id="editor" name="{{ $key }}" value="{{ $model->$key }}"> </input>
                                                <div class="valid-feedback">
                                                    Looks good!
                                                </div>
                                            </div>
                                    ...
                                    @else
                                        <div class="row">
                                            <div class="col-md-8 mb-3">
                                                <label class="mb-2" for="validationCustom01">{{ $item }}</label>
                                                <input type="text" class="form-control" id="validationCustom01" name="{{ $key }}" value="{{ $model->$key ?? old($key) }}">
                                                @if($errors->has($key))
                                    ...
                                    @endif
                                @endforeach
                                <button class="btn btn-primary" type="submit">Cập nhật {{ $title_web }}</button>
                            </form>
                        </div>
                    </div>
                </div>
